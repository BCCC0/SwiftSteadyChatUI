...
# Trim 5s of app-launch preamble and 1s of app-termination tail.
# RE-ENCODE (not -c copy): simctl's H.264 has sparse keyframes, so a copy seek
# snaps to the previous keyframe and the 5s/1s cut is mostly ignored. Seeking
...
trim_video() {
  ...
  local end
  end=$(awk -v d="$dur" -v t="1" 'BEGIN { print d - t }')
